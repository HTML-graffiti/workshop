...
if( !empty($_POST['btn_confirm']) ) {
	...
	// 件名を設定
	$auto_reply_subject = 'Workshop | The Things I (We) Own';
	...
	// 運営側へ送るメールの件名
	$admin_reply_subject = "Workshop | The Things I (We) Own";

	// 本文を設定
	$admin_reply_text .= "Workshop | The Things I (We) Own\n\n\n";
	$admin_reply_text .= "名前\n" . $_POST['name'] . "\n";
...
<title>Workshop | The Things I (We) Own</title>
...
<p>
	会場：BnA Alter Museum にて | 
	2022年8月13日(土) - 16日(火) |
	開催時間 10:00 - 18:00（事前予約制）
</p>
<p>自分の所有するものを記録するホームページを作るワークショップを開催します。</p>
</div>
<div id="form">
<?php if( $page_flag === 1 ): ?>
...
<section>
<p>
	面倒になりがちな所有物の管理／整理整頓を、楽しく便利に自分らしい方法で継続する一つの方法として、所有するものを記録するホームページを作るワークショップです。
	<br/>このワークショップで、所有するものをホームページにまとめるささやかな喜びを共有できるとうれしいです。
</p>
<p>Links | 
	<a href="https://newlifecollection.com/ca94/1313/p-r-s/" target="_blank">ワークショップのタイムテーブル</a> | 
	<a href="/org/template/" target="_blank">Demo Page</a> | 
	<a href="/pehu/org/" target="_blank">The Things ∧° ┐ Own</a> | 
</p>
<hr/>
<h1>ワークショップを予約する</h1>
<p><u>参加費：2500円＋税</u> (お支払いは、ワークショップ終了後 BnA Alter Museum 受付にて)</p>
<form action="" method="post">
...
<p><u>所要時間：約2時間半</u>  (日程は 2022年8月13日(土) - 16日(火) | 開始時間は 10:00 ~ 15:00 の間でご指定ください。) </p>
<input type="date" name="date" value="<?php if( !empty($_POST['date']) ){ echo $_POST['date']; } ?>" min="2022-08-13" max="2022-08-16"　required><br/>
<input id="time" type="text" name="time" placeholder="ワークショップ希望開始時間" value="<?php if( !empty($_POST['time']) ){ echo $_POST['time']; } ?>" required>
</hr>
<p><br/><u>作りたいホームページの内容</u>  (複数名でワークショップに参加したい場合は、その旨も合わせてご記入ください。) </p>
<textarea name="contact" placeholder="ワークショップをスムーズに進めるため、載せたい所有物や、リストの分け方、絞り込みカテゴリー、作りたいホームページについてなど、考えていることを簡単にご記入ください。" required><?php if( !empty($_POST['contact']) ){ echo $_POST['contact']; } ?></textarea>
